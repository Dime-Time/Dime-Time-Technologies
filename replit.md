...
- Crypto: ships as labeled **PREVIEW** (founder decision 2026-07-25) — Crypto page banner says purchases are simulated; prices are now live via Coinbase's public API. Real integration = per-user Coinbase OAuth (users' own accounts; the single-account custody model is permanently off the table — see `.agents/memory/coinbase-crypto-preview.md`). Founder background action (not launch-blocking): request OAuth access via Coinbase's developer-interest form — new OAuth client creation is partner-gated as of 2026-07, so there's no self-serve "create app" button yet. Label reaches web on next republish; native apps with build ≥210.
