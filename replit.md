...
- **iOS Build Number Source of Truth**: `codemagic.yaml` overwrites `CFBundleVersion` at build time using `$(({BUILD_NUMBER} + BUILD_NUMBER_OFFSET))`. This makes every Codemagic build produce a strictly-monotonic CFBundleVersion automatically — do NOT bump `Info.plist`'s CFBundleVersion expecting Codemagic to honor it. If Apple ever rejects a build as duplicate, raise `BUILD_NUMBER_OFFSET` in `codemagic.yaml` (Step: "Bump iOS build number"), not Info.plist. `MARKETING_VERSION` / `CFBundleShortVersionString` in Info.plist IS the source of truth (Codemagic does not touch it).
