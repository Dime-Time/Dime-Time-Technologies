...
- Stripe account live; prod has `ENABLE_STRIPE_ACH` + `ENABLE_REAL_TRANSFERS` ON (founder decision; public protected by the per-user launch limits + admin block list).
- LinkedIn launch post is INTENTIONALLY HELD (founder decision 2026-07-09) until two milestones land: (1) real money movement working for users, (2) Plaid debt import live in production. Stripe side DONE: FC approved 2026-07-24, founder completed the full bank-link flow (Test A) 2026-07-25. Plaid Liabilities NOT active yet: Order Form signed 2026-07-24 but the authoritative link-token probe still returns INVALID_PRODUCT (2026-07-25) — activation typically hours-to-days; escalate to Plaid rep ~2026-07-29 if unchanged. Remaining: Plaid activation (external wait) + one real debt import.
- Crypto: ships as labeled **PREVIEW** (founder decision 2026-07-25) — Crypto page banner says purchases are simulated; prices are live via Coinbase's PUBLIC price API (no credentials). The app contains **no authenticated Coinbase client** (removed 2026-07-25): `COINBASE_API_KEY`/`COINBASE_API_SECRET` secrets are unused by code, and the company Advanced Trade key must never serve customers (single-account custody permanently rejected — see `.agents/memory/coinbase-crypto-preview.md`). Real crypto ships only after a licensed provider approves in writing: outreach submitted 2026-07-25 to Coinbase (OAuth whitelisting), Alpaca Broker API, and Zero Hash — founder monitoring tim@dime-time.com. Preview reaches web on next republish; native apps with build ≥210.
...
- `ENABLE_REAL_TRANSFERS` alone is NOT sufficient to move public money: every real ACH **transfer/debt-payment** debit passes through `storage.reserveRealStripeAchDebit()` — per-user admin block list (`users.realTransfersBlocked`, default false = allowed for everyone) + first transfer ≤ $1 + daily total ≤ $5 + daily count ≤ 1 + duplicate-pending guard, all inside one transaction with an advisory lock. Every allow AND block decision writes a `real_transfer_audit_logs` row. Blocks never call Stripe.
- Test key mode → simulation path (`status="simulated"`, no settlement). Admin can block a user instantly via `/api/admin/users/:id/real-transfers` (takes effect on next attempt, no restart).
- Subscription billing debits ($2.99/mo, `ENABLE_SUBSCRIPTIONS`) are charged by Stripe Billing and intentionally do NOT pass the transfer block-list gate; their own guards: consent-first ordering, Idempotency-Key + per-user subscribe lock, duplicate-sub 409, webhook-driven entitlement revocation.
...
## Internal Admin
Gated by `ADMIN_USER_IDS` secret (fails closed when empty). `/admin` page: Transfers, Stripe Webhooks, and Real Money (block/unblock) tabs. `/api/user` piggybacks `_isAdmin`. Do NOT re-add the removed Stripe Diagnostics tab (verdict logic preserved in `shared/stripeVerdict.ts`).
