...
Two follow-on lessons (2026-07-28, round-up bank-connect audit):
- A ref guard keyed by token in the SAME component that owns the token state races: the effect can fire with the new token but the previous Link instance's stale `ready`/`open`, burning the guard so the pane never opens (hidden `display:none` iframe). The safe pattern is a dedicated launcher component mounted per token (`{token && <Launcher key={token} …/>}`) with a plain boolean openedRef inside; clear the token on exit/success so a re-click mints a fresh launcher.
- Plaid's "link-initialize.js embedded more than once" console warning is caused by React.StrictMode double-running react-plaid-link's useScript effect (script tag removed mid-load, second appended). Fix: embed `https://cdn.plaid.com/link/v2/stable/link-initialize.js` once statically in index.html — react-plaid-link's `checkForExisting: true` reuses it and never injects its own. Don't remove that tag.
