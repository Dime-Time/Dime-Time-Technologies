...
**2026-07-28 status:** Alpaca is the chosen direction for embedded crypto round-ups (warm lead — Director of Sales requested intro call; ZeroHash passed: ~$25k/mo minimums). Founder's full concept brief + go/no-go criteria + 56 call questions live in `attached_assets/Pasted-DIME-TIME-ALPACA-EMBEDDED-CRYPTO-CONCEPT-*.txt` — read it before any Alpaca work. Hard gate: NO implementation (no tables, webhooks, credentials, logos, legal changes) until Alpaca supplies written pricing, funding architecture (prefer direct bank-pull, money never touches Dime Time), compliance split, and pilot approval. Timing also gated behind Google Play approval + native build 210.
