...
Google rejected v209 solely because the Sign-in-details credentials didn't authenticate ("Login credentials are incorrect"). Evidence screenshot (zoomed) shows the reviewer typed the CORRECT email (tim@dime-time.com) and got the app's genuine 401 error banner. Verified at diagnosis time: prod CORS accepts the Android WebView origin (live curl with Origin https://localhost → 401 JSON + correct ACAO header, so the app↔API path works), founder account exists + email-verified in prod, and the login route has only two 401 paths (no-such-user / wrong password) → **the password on file with Google was stale or mistyped** — prime suspect: the founder password rotation that was pending after the chat-screenshot exposure, done without updating Play Console.
**Lessons:** (1) any founder-password rotation MUST same-day update reviewer credentials in BOTH App Store Connect and Play Console; (2) login email lookup is exact-match case-sensitive — reviewer credentials must be lowercase exactly; (3) fix path = incognito login test at dime-time.com with the exact strings → Play Console → App content → App access → update → resubmit from Publishing overview. No new .aab, no appeal — this rejection type is administrative, not a policy strike.
