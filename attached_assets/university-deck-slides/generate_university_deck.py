...
18 core slides + 3 appendix slides = 21 total. 16:9, native editable text,
embedded speaker notes on every slide. Built per UNIVERSITY-DECK-BRIEF.txt.
"""
...
# ================= SLIDE 3 — HOSPITALITY =================
s = slide(WHITE)
header(s, "What Hospitality Taught Me", "Hospitality Was Founder Training")
lessons = [
    "Reading customer needs quickly",
    "Performing under pressure",
    "Communicating complex information clearly",
    "Building trust",
    "Recovering when something goes wrong",
    "Managing timing and operational details",
    "Delivering consistent experiences",
    "Working within high standards",
    "Leading without formal authority",
]
col1 = box(s, 0.9, 2.05, 5.7, 4.3)
col2 = box(s, 6.8, 2.05, 5.7, 4.3)
for i, t in enumerate(lessons):
    tgt = col1 if i < 5 else col2
    para(tgt, "•  " + t, 19, DARK, first=(i in (0, 5)), space_after=13)
bar = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.9), Inches(5.75), Inches(11.6), Inches(1.05))
bar.fill.solid(); bar.fill.fore_color.rgb = PURPLE
bar.line.fill.background(); bar.shadow.inherit = False
tf = box(s, 1.25, 5.95, 11.0, 0.75)
para(tf, "A financial application also depends on trust, reliability, clarity and a carefully designed customer experience.", 18, WHITE, bold=True, first=True)
footer(s)
notes(s, "Pick two or three of these and tell a real 30-second story for each — a table that went wrong and how you recovered; explaining a $300 wine to a guest in one sentence; a Saturday night rush.\n\nThen make the bridge explicit: a fintech app is a hospitality product. Users hand you their bank connection the way a guest hands you their evening — the trust is the product. Reliability, clarity, and recovery-when-things-break are the same skills.\n\n[45-60 MIN] Great slide to expand with audience interaction: ask students what their current jobs are teaching them.\n[20-MIN VERSION] One story only, 60 seconds total.")

# ================= SLIDE 4 — CLASSROOM FOUNDATION =================
s = slide(LIGHT)
...
# ================= SLIDE 7 — FROM IDEA TO SYSTEM =================
s = slide(WHITE)
header(s, "From Idea to System", "A Fintech Product Is More Than an App Screen")
flow = ["USER EXPERIENCE", "BANK CONNECTION", "TRANSACTION ANALYSIS", "ROUND-UP CALCULATION", "USER AUTHORIZATION", "ACH PAYMENT", "TRANSFER TRACKING & RECONCILIATION"]
y = 1.95
for i, t in enumerate(flow):
    chip(s, 0.9, y, 4.7, 0.52, PURPLE if i % 2 == 0 else DEEP, t, 15)
    if i < len(flow) - 1:
        tf = box(s, 2.9, y + 0.44, 0.7, 0.32)
        para(tf, "↓", 14, DARK, bold=True, align=PP_ALIGN.CENTER, first=True)
    y += 0.72
tf = box(s, 6.3, 1.95, 6.3, 4.9)
para(tf, "Building Dime Time required:", 19, DARK, bold=True, first=True, space_after=8)
reqs = [
    "Mobile and web interfaces  •  authentication",
    "Database design  •  bank connectivity",
    "Transaction processing  •  ACH payment infrastructure",
    "User authorization records  •  security controls",
    "Transfer status tracking  •  error recovery",
    "Privacy and legal documentation",
    "App Store compliance",
]
for t in reqs:
    para(tf, "•  " + t, 17, DARK, space_after=9)
para(tf, "The app screen is the visible 10%. The other 90% is the system underneath it.", 17, DEEP, bold=True, space_after=0)
footer(s)
notes(s, "This slide reframes what 'building an app' means. Students often picture the interface. The interface was the easy part.\n\nWalk the left-side pipeline top to bottom: every purchase flows through seven stages before a single dollar reaches a debt. Each stage is its own engineering and compliance problem.\n\nFor the MIS students specifically: this is systems architecture in the wild — exactly what their coursework describes, applied to real money.\n\n[45-60 MIN] Expand on one stage in depth — reconciliation is a good choice because nobody thinks about it until money goes missing.\n[20-MIN VERSION] Summarize in 45 seconds: 'seven stages, each one has to be right.'")

# ================= SLIDE 8 — NONTECHNICAL FOUNDER =================
s = slide(LIGHT)
header(s, "Building Without an Engineering Background", "Learning to Build Without a Traditional Engineering Background", tsize=28)
tf = box(s, 0.9, 1.95, 7.3, 4.7)
for i, t in enumerate([
    "Began with the business problem, not the programming language",
    "Learned how software components fit together",
    "Used AI-assisted development to accelerate learning and execution",
    "Broke large problems into smaller, testable tasks",
    "Learned to verify outputs instead of accepting them blindly",
    "Used screenshots, tests, logs and controlled experiments as evidence",
    "Developed enough technical literacy to make product decisions and question unsafe assumptions",
]):
    para(tf, "•  " + t, 18, DARK, first=(i == 0), space_after=11)
pnl = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(8.5), Inches(2.15), Inches(4.1), Inches(4.1))
pnl.fill.solid(); pnl.fill.fore_color.rgb = DARK
pnl.line.fill.background(); pnl.shadow.inherit = False
tf = box(s, 8.85, 2.55, 3.4, 3.4)
para(tf, "CORE LESSON", 14, GOLD, bold=True, first=True, space_after=10)
para(tf, "AI can accelerate execution —", 19, WHITE, bold=True, space_after=6)
para(tf, "but it does not replace founder judgment, testing, accountability or domain understanding.", 18, LAV2)
footer(s)
notes(s, "Be specific and honest here. You did not become a software engineer. You became technically literate enough to direct the work, verify it, and question it.\n\nThe verification habit is the real story: never accepting 'it works' without evidence — screenshots, logs, tests, controlled experiments. That habit came from hospitality and the Navy, not from a CS degree.\n\n[DO NOT OVERSTATE] Never claim AI built or operated the company on its own. The claim is 'AI-assisted' — you directed, decided, and carried the responsibility.\n[PAUSE HERE] Students will want to ask about AI tools — decide whether to take those questions now or defer to slide 15, which covers it fully.")

# ================= SLIDE 9 — THE TECHNOLOGY =================
s = slide(WHITE)
header(s, "The Technology", "The System Behind Dime Time")
cards = [
    ("FRONT END", ["React", "TypeScript", "Capacitor mobile application"], PURPLE),
    ("BACK END", ["Express / Node.js", "PostgreSQL", "Drizzle ORM"], DEEP),
    ("FINANCIAL INFRASTRUCTURE", ["Plaid — approved bank-connectivity products", "Stripe ACH", "Stripe Financial Connections — under review", "Mercury — business bank & payout destination"], DARK),
    ("OPERATIONS", ["Replit development environment", "Codemagic mobile build workflow", "Apple App Store", "Google Play preparation"], DEEP),
]
positions = [(0.9, 1.95), (7.0, 1.95), (0.9, 4.25), (7.0, 4.25)]
for (title, items, color), (x, y) in zip(cards, positions):
    c = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(x), Inches(y), Inches(5.7), Inches(2.1))
    c.fill.solid(); c.fill.fore_color.rgb = color
    c.line.fill.background(); c.shadow.inherit = False
    tf = box(s, x + 0.3, y + 0.16, 5.1, 1.85)
    para(tf, title, 15, GOLD if color == DARK else LAV2, bold=True, first=True, space_after=5)
    for t in items:
        para(tf, "•  " + t, 14.5, WHITE, space_after=3)
tf = box(s, 0.9, 6.5, 11.8, 0.5)
para(tf, "Partner names indicate service relationships, not endorsements.", 14, DEEP, bold=True, first=True)
footer(s)
notes(s, "Keep this quick for a mixed audience; go deeper only for MIS/CS-heavy rooms.\n\nThe stack story: React and TypeScript for the interface, one codebase wrapped by Capacitor for iOS, Express and PostgreSQL behind it. Plaid handles bank connectivity for approved products; Stripe runs the ACH rails; Mercury is the business bank where payouts land — a banking relationship, not a software integration.\n\n[DO NOT OVERSTATE] Stripe Financial Connections is UNDER REVIEW — say 'under review,' not 'approved.' Coinbase is not in this stack at all — it is planned only, not available to users.\n[45-60 MIN] MIS students may ask why Capacitor over native Swift — answer: one founder, one codebase, faster iteration.")

# ================= SLIDE 10 — SAFETY BEFORE SCALE =================
s = slide(LIGHT)
...
rows = [
    ("2014–2025", "The idea & classroom era — concept, brand and an early demo shaped through business education", LAV2, DARK),
...
for date, desc, fill, tc in rows:
    ...
    chip(s, 1.95, y, 1.85, 0.62, fill, date, 15, tcolor=tc)
...
footer(s)
notes(s, "Walk this top to bottom — it's the spine of the whole talk.\n\nThe idea sat un-built from 2014 to 2025: over a decade of no funding, no team, no code. First commit August 3, 2025. Live on the App Store June 29, 2026 — under eleven months later. First real dollar moved July 7. Loop proven bank-to-bank July 21.\n\nThe teaching point: the gap between 2014 and 2025 is the most common startup story there is — the idea waiting for its builder. What changed wasn't the idea; it was the decision to start, plus tools that made a solo build feasible.\n\n[PAUSE HERE] Let the 11-year gap sink in before moving on.\n[DO NOT OVERSTATE] No invented dates for the classroom concept or patent filing — the dated milestones on this slide are the verified ones.")
...
# ================= SLIDE 13 — INVISIBLE WORK =================
s = slide(LIGHT)
header(s, "What Did Not Happen Overnight", "The Invisible Work Behind a Launch")
work = [
    "App Store reviews and resubmissions",
    "Resolving a duplicate-app rejection",
    "Payment-provider applications",
    "Bank-connectivity approvals",
    "Security reviews",
    "Legal documentation",
    "Email reliability fixes",
    "OAuth and universal-link work",
    "Mobile screenshot preparation",
    "Version and build management",
    "Android permission & signing preparation",
    "Waiting through third-party review queues",
]
col1 = box(s, 0.9, 2.0, 5.7, 4.0)
col2 = box(s, 6.8, 2.0, 5.7, 4.0)
for i, t in enumerate(work):
    tgt = col1 if i < 6 else col2
    para(tgt, "•  " + t, 17, DARK, first=(i in (0, 6)), space_after=10)
bar = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.9), Inches(6.0), Inches(11.6), Inches(1.05))
bar.fill.solid(); bar.fill.fore_color.rgb = DEEP
bar.line.fill.background(); bar.shadow.inherit = False
tf = box(s, 1.2, 6.17, 11.0, 0.8)
para(tf, "The public sees a launch date. The founder experiences hundreds of small decisions, failures, corrections and verification steps.", 17, WHITE, bold=True, first=True)
footer(s)
notes(s, "Read three or four items off this list, then stop and let the length of it speak.\n\nThe theme: none of this appears in a product screenshot, and all of it was mandatory. Weeks were spent inside review queues you cannot accelerate — Apple, payment providers, banking partners. Protecting real money movement while continuing to build was a daily constraint, not a one-time task.\n\nFor students, the honest framing is: 'the boring middle' is most of the job. Persistence through the un-glamorous work is the differentiator, not the initial idea.\n\n[20-MIN VERSION] 45 seconds — read the banner line and move on.\n[45-60 MIN] Pick one item (the duplicate-app rejection is a good one) and tell the full story.")

# ================= SLIDE 14 — SETBACKS TABLE =================
s = slide(WHITE)
...
tf = box(s, 0.9, 6.75, 11.8, 0.5)
para(tf, "No guaranteed approval dates or launch dates — external review timelines are not ours to promise.", 14, DEEP, bold=True, first=True)
footer(s)
...
footer(s, light=True)
notes(s, "End with energy and turn it back to them: 'What will you build?' is a genuine question, not a rhetorical flourish.\n\nSuggested close: 'A year ago this was an idea in a folder. Today it's a live app with proven financial rails. The gap between those two states wasn't talent or money — it was starting, and then refusing to stop. Some of you have the idea already. What will you build?'\n\nThen open Q&A. Common questions to be ready for: How much did it cost? (development costs were kept minimal as a solo founder), How long did Apple review take? (multiple cycles — see slide 13), Are you hiring? (not yet — post-funding), Can I try the app? (yes — it's on the App Store today).\n\n[Reserve 15–20 minutes for Q&A in a 60-minute class session.]")

# ================= SLIDE 19 — APPENDIX A =================
s = slide(LIGHT)
header(s, "Appendix A — For Class Discussion", "Fintech Terminology")
terms = [
    ("ACH", "The U.S. bank-to-bank transfer network; settles in days, not seconds"),
    ("Bank linking", "Letting an app securely connect to a user's bank account"),
    ("Financial Connections", "Stripe's product for verified bank account linking"),
    ("Payment authorization", "Recorded user consent before money moves"),
    ("Idempotency", "Ensuring a repeated request cannot cause a duplicate payment"),
    ("Webhook", "A provider's callback that reports events (like settlement) to the app"),
    ("Feature flag", "A switch that turns functionality on or off without redeploying"),
    ("Allowlist", "An explicit list of approved users — everyone else is denied"),
    ("Production vs. sandbox", "Real money and real data vs. a safe simulated environment"),
    ("Settlement", "The moment transferred funds actually arrive and are final"),
]
col_x = [0.9, 6.9]
for i, (term, defn) in enumerate(terms):
    x = col_x[0] if i < 5 else col_x[1]
    y = 2.0 + (i % 5) * 0.92
    tf = box(s, x, y, 5.7, 0.9)
    para(tf, term, 16, DEEP, bold=True, first=True, space_after=1)
    para(tf, defn, 14, DARK, space_after=0)
footer(s)
notes(s, "Optional slide — use in 60-minute class sessions or when the audience is less familiar with fintech vocabulary.\n\nTwo terms worth 30 extra seconds each if time allows:\n\nIdempotency — the reason you can double-click 'Pay' without being charged twice. Every serious payment system is built around this idea, and students rarely hear the word before their first fintech job.\n\nSettlement — ACH is not instant. Money 'moving' and money 'arrived' are different states separated by days, and the entire transfer-tracking system exists because of that gap.")

# ================= SLIDE 20 — APPENDIX B =================
s = slide(WHITE)
header(s, "Appendix B — For Class Discussion", "Classroom Discussion Questions")
qs = [
...
# ================= SLIDE 21 — APPENDIX C =================
s = slide(LIGHT)
header(s, "Appendix C — Honest Labels", "Verified Project Status")
groups = [
    ("LIVE", GREEN, ["iOS v1.0.5, build 207 on the Apple App Store", "Core product interface", "Production access for approved Plaid products", "Stripe ACH infrastructure"]),
    ("PROVEN IN CONTROLLED LIVE TEST", DEEP, ["$1.00 ACH payment", "$0.99 Mercury payout", "Bank-to-bank financial loop"]),
    ("BUILT • FEATURE-GATED", PURPLE, ["Core $2.99 subscription functionality", "Real-transfer functionality behind safeguards"]),
    ("PENDING", GOLD, ["Stripe Financial Connections public bank-linking approval", "Plaid Liabilities approval", "Google Play launch process"]),
    ("PLANNED", DARK, ["Coinbase / Bitcoin allocation", "Pro and Premium subscription tiers", "Broader public acquisition"]),
]
xs = [(0.55, 2.6), (3.3, 2.6), (6.05, 2.35), (8.55, 2.3), (11.0, 1.95)]
for (label, color, items), (x, w) in zip(groups, xs):
    chip(s, x, 1.95, w, 0.75, color, label, 13 if len(label) > 12 else 15, tcolor=(DARK if color == GOLD else WHITE))
    tf = box(s, x, 2.85, w, 3.9)
    for i, t in enumerate(items):
        para(tf, "•  " + t, 14, DARK, first=(i == 0), space_after=7)
tf = box(s, 0.55, 6.75, 12.2, 0.5)
para(tf, "Statuses are kept deliberately distinct — the honesty of the labels is part of the company's operating discipline.", 14, DEEP, bold=True, first=True)
footer(s)
notes(s, "Optional closing appendix — most useful when faculty, administrators, or investor-minded students are in the room.\n\nThe meta-point matters more than the individual items: the company maintains five distinct status labels and refuses to blur them. 'Live,' 'proven in a controlled test,' 'built but gated,' 'pending,' and 'planned' are different claims, and treating them as different claims is what keeps communication trustworthy — with users, with partners, with regulators, and with future investors.\n\nIf someone asks 'when does pending become live?' — the honest answer is that those timelines belong to external reviewers, not to us.")
